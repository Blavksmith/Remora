...
import { Sparkles, Save, Loader2, UploadIcon } from "lucide-react";
...
- Prioritaskan konsep paling penting terlebih dahulu`;
  ...
  const handleGenerate = async () => {
    ...
    try {
      const generatedFlashcards = await generateFlashcardsWithGemini(
        title,
        notes
      );
      ...
      toast({
        title: "Error",
        description:
          error.message || "Terjadi kesalahan saat generate flashcard",
        variant: "destructive",
      });
    } finally {
      setGenerating(false);
        ...
        >
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600 text-lg ml-[60px]">
              Paste catatanmu dan biarkan AI membuat flashcard berkualitas
              untukmu
            </p>
          </div>
...
• Tulis dengan jelas dan sistematis"
                      className="min-h-[350px] resize-none border-2 border-emerald-100 focus:border-emerald-500 rounded-xl"
                      value={notes}
                  ...
                  >
                    <Button
                      ...
                      disabled={generating}
                      className="w-full gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white shadow-lg rounded-xl py-6 font-semibold text-base"
                    >
                      {generating ? (
                        <>
                  ...
                  {generating && (
                    ...
                    >
                      <p className="text-sm text-gray-700 text-center">
                        AI sedang menganalisis catatanmu dan membuat pertanyaan
                        yang membantu mengingat...
                      </p>
                    </motion.div>
                  )}
                </CardContent>
                ...
                <CardContent className="pt-6">
                  {flashcards.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      >
                        <Sparkles className="h-16 w-16 mx-auto mb-4 text-emerald-400" />
                      </motion.div>
                    ...
                    <>
                      <div className="space-y-3 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                        {flashcards.map((card, index) => (
                          ...
                          >
                            <Card className="hover:shadow-md transition-all border border-emerald-100 rounded-xl overflow-hidden group">
                              <CardContent className="pt-4">
                                <div className="space-y-3">
                                  <div className="flex items-center justify-between">
                                    <p className="font-semibold text-xs text-emerald-600 uppercase tracking-wider">
                                      Pertanyaan {index + 1}
                                    </p>
                                    <div className="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center group-hover:bg-emerald-600 transition-colors">
                                      <span className="text-xs font-bold text-emerald-700 group-hover:text-white transition-colors">
                                        {index + 1}
                                      </span>
                                    </div>
                                  </div>
                      ...
                      >
                        <Button
                          onClick={handleSave}
                          disabled={saving}
                          className="w-full gap-2 bg-gradient-to-r from-gray-700 to-gray-600 hover:from-gray-800 hover:to-gray-700 text-white shadow-lg rounded-xl py-6 font-semibold text-base"
                        >
                          {saving ? (
                            <>
                              <Loader2 className="h-5 w-5 animate-spin" />
